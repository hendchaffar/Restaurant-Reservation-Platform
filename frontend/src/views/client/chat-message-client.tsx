...
export default function ChatMessagesClient() {
  ...
  const { chats } = useAxios("chats/chatperuser", "GET", {}, "chats", true, {
    client: {
      id: user?.id,
    },
    manager: { id: currentCompany?.manager.id },
  });
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-gray-900 text-black dark:text-white">
      <div className="p-4 border-b dark:border-neutral-600 shadow-sm sticky top-0 z-10 bg-white dark:bg-gray-800">
        <div className="flex gap-4   items-center">
          ...
          <div className="flex gap-2 items-center">
            ...
            <small className="font-light text-xs ">Manager</small>
            </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 bg-gray-50 dark:bg-gray-800">
        {(chats.responseData as Chat)?.messages?.map((message) => (
          <div
            key={message.id}
            className={`mb-4 flex ${
          ...
          >
            ...
            >
              <p>{message.content}</p>
              <small className="mt-1 opacity-70">{message.timestamp.toDateString()}</small>
            </div>
          </div>
        ))}
