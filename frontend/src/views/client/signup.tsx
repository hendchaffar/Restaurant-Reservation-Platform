...
const SignUpPage = () => {
  ...
  const handleSubmit = () => {
    if (isFormValid) {
      signupdata.submitRequest(form);
    }
  };

  useEffect(() => {
    if (signupdata.responseData && !signupdata.error) {
      navigate("/client");
    } else if (signupdata.error) {
      console.error("Sign up failed:", signupdata.error);
    }
  }, [signupdata.responseData, signupdata.error]);
        ...
        {roles.map((r) => (
          ...
          <Input
            ...
            endContent={
              ...
              >
                {isVisible ? (
                  <EyeSlashFilledIcon className="text-gray-400" size={18} />
                ) : (
                  <EyeFilledIcon className="text-gray-400" size={18} />
                )}
              </button>
            }
